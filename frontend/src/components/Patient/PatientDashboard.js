...
function PatientDashboard() {
  ...
  const searchDoctors = async () => {
    ...
    try {
      ...
      if (res.data.doctors.length === 0) {
        setShowNoDoctors(true);
        setTimeout(() => setShowNoDoctors(false), 2500);
      }
    } catch (err) {
  ...
  const filteredSchedule = useMemo(() => {
    ...
    return Object.entries(selectedDoctor.schedule)
      .filter(([dateStr]) => {
  ...
  const hasAvailableSlots = useMemo(() => {
    return filteredSchedule.some(([date, slots]) => {
      const available = slots.filter(
        (slot) => !doctorBookedSlots[date]?.includes(slot)
      );
    ...
    >
      ...
      <Container maxWidth="lg" sx={{ mt: 4, pb: 4 }}>
        ...
        >
          ...
          <Typography sx={{ mb: 2 }}>
            Try our AI virtual doctor to get a preliminary diagnosis report
        ...
        {showNoDoctors && (
          <Alert
            severity="warning"
            sx={{ maxWidth: 400, margin: "32px auto 32px 0" }}
          >
          ...
          >
            ...
            <DialogContent>
              {!hasAvailableSlots ? (
                <Alert severity="warning" sx={{ mt: 2 }}>
                  No available slots for booking in the next 7 days.
                </Alert>
              ) : (
                <RadioGroup
                  name="slot"
                  value={`${bookingDetails.date}__${bookingDetails.time}`}
                  onChange={(e) => {
                ...
                >
                  {filteredSchedule.map(([date, slots]) => {
                    const avail = slots.filter(
                      (s) => !doctorBookedSlots[date]?.includes(s)
                    );
            ...
            <DialogActions sx={{ p: "16px 24px" }}>
              ...
              <Button
                ...
                disabled={!bookingDetails.date || !hasAvailableSlots}
                variant="contained"
              >
                Confirm Booking
              </Button>
        ...
        <Paper sx={{ p: 3, borderRadius: 3 }}>
          ...
          <Box sx={{ borderBottom: 1, borderColor: "divider" }}>
            <Tabs
              ...
              onChange={(e, val) => setAppointmentTab(val)}
